...
XYZAdminCtrls.controller('loginCtrl', ['$location', '$timeout', '$scope', '$http', 'AuthService',
    function ($location, $timeout, scope, $http, AuthService) {
        scope.auth = AuthService.isLogged() ? $location.path('/sellers') : false;
        scope.login = {};
        scope.loginSend = function (data) {
            $('.login-login').removeClass('failed');
            $('.login-password').removeClass('failed');

            $http.get('/admin/sign-in', {params: {login: data.login, password: data.password}}).then(function (resp) {
...
XYZAdminCtrls.controller('userCtrl', ['$location', '$timeout', '$scope', '$http', '$rootScope', '$q', 'getContent', 
    function ($location, $timeout, scope, $http, rs, $q, getContent) {
        scope.isActive = {
            users: true
        };

        scope.users = getContent.users.data.data;
        scope.approved = function (user, i) {
            $http.get('/approved', {params: {username: user.username}}).then(function (resp) {
                scope.users[i] = [resp.data.data][0];
            }, function (err) {
            })
        };
        ...
        scope.rejectUser = function (data, i) {
            $http.get('/reject', {params: {username: scope.check.user.username, reject_reason: data}}).then(function (resp) {
                scope.users[scope.check.i] = [resp.data.data][0];
                scope.reject = false;
            }, function (err) {
            })
        };
...
XYZAdminCtrls.controller('claimCtrl', ['$location', '$timeout', '$scope', '$http', '$rootScope', '$q', 'getContent', 'ModalService',
    function ($location, $timeout, scope, $http, rs, $q, getContent, ModalService) {
        scope.isActive = {
            claims: true
        };

        scope.agency = getContent.agency.data.data;
...
XYZAdminCtrls.controller('jobCtrl', ['$location', '$timeout', '$scope', '$http', '$rootScope', '$q', 'getContent', 'ModalService',
    function ($location, $timeout, scope, $http, rs, $q, getContent, ModalService) {
        scope.isActive = {
            jobs: true
        };
...
XYZAdminCtrls.controller('sellerCtrl', ['$location', '$timeout', '$scope', '$http', '$rootScope', '$q', 'getContent', 'ModalService', '$routeParams', 'ngDialog', 'notify','cfpLoadingBar',
    function ($location, $timeout, scope, $http, rootScope, $q, getContent, ModalService, routeParams, ngDialog, notify, cfpLoadingBar) {
        scope.sellers = getContent.sellers.data;
        scope.service = getContent.service.data.data;
        scope.locations = getContent.locations.data.data;

        scope.rejectApproveProfile = function(status, sellecrId){
            cfpLoadingBar.start()
            $http.get('/admin/registration/'+status+'/'+sellecrId).success(function(){
                cfpLoadingBar.complete()
                notify({message: 'Seller with id ' + sellecrId + ' has been succesfully '+ (status == 'reject' ? 'rejected' : 'approved'), position: 'right'});
            })
        };
